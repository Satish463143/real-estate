      ...
      <div className="max-w-7xl mx-auto px-6 md:px-10">
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7">
            {[1, 2, 3].map((skeleton) => (
              <div key={skeleton} className="animate-pulse bg-gray-100 h-96 rounded-2xl w-full" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7">
            {properties.map((prop: any) => {
              ...
              return (
                <Link
                  ...
                  key={prop.id}
                  className="bg-white rounded-2xl border border-gray-100 shadow-[0_4px_16px_rgba(10,22,40,0.09)] overflow-hidden hover:-translate-y-1.5 hover:shadow-[0_16px_48px_rgba(10,22,40,0.16)] transition-all duration-300 cursor-pointer group flex flex-col"
                >
                  {/* Image */}
                  <div className="relative h-56 overflow-hidden shrink-0">
                    ...
                    {/* Badge */}
                    <span className="absolute top-4 left-4 inline-flex items-center px-2.5 py-1 rounded-full text-[0.65rem] font-bold tracking-wide uppercase bg-accent text-primary">
                      Featured
                    </span>
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    <div className="flex items-center gap-1.5 mb-2">
                      ...
                      <span className="text-xs text-gray-400 truncate max-w-[150px]">{prop.location?.city}, {prop.location?.country}</span>
                      <span className="ml-auto text-[0.68rem] bg-gray-100 text-gray-600 px-2.5 py-0.5 rounded-full font-semibold uppercase tracking-wider">{prop.propertyType}</span>
                    </div>

                    <h3 className="font-display text-lg font-bold text-primary mb-4 leading-snug line-clamp-1">
                      {prop.title}
                    </h3>

                    <div className="flex items-center gap-5 pt-4 mt-auto border-t border-gray-100">
                      <span className="flex items-center gap-1.5 text-gray-400 text-xs font-medium">
                        <BedIcon /> {prop.bedrooms} Beds
                      </span>
                      <span className="flex items-center gap-1.5 text-gray-400 text-xs font-medium">
                        <BathIcon /> {prop.bathrooms} Baths
                      </span>
                      <span className="flex items-center gap-1.5 text-gray-400 text-xs font-medium uppercase">
                        <AreaIcon /> {prop.areaSize} {prop.areaSizeUnit}
                      </span>
                    </div>
                  </div>
                </Link>
