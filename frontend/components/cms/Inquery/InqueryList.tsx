...
import { useListAllQuery, useDeleteInquiryMutation } from '../../api/inquiry.api';
    ...
    const handleSearchChange = (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const deleteData=async(id:string)=>{
        try{
          ...
          Swal.fire({
            title:"Deleted!",
            text:"Inquery deleted successfully",
            icon:"success"
          })
        }catch(exception){
          Swal.fire({
            title:"Error!",
            text:"Cannot delete inquery at this moment",
            icon:"error"
          })
        }    
      }
  return (
    <div className='admin_margin_box'>
      <div className='admin_titles'>
        <AdminTitle url='/admin/inquery' label1=' Inquery List' label2='' />
      ...
      <div className='blog_table'>        
        <table border={2}>
          ...
          <tbody>
            {isLoading ? (
            ...
            ) : inquery && inquery.length > 0 ? (
              inquery.map((row: any, index: number) => (
                <tr key={index}>
                  <td className="table_sn">{index + 1}</td>
                  <td>{row.name}</td>
                  <td>{row.email}</td>
                  <td>{row.phone}</td>
                  <td>{truncateContent(row.message, 15)}</td>
                  <td>{row.status}</td>
                  <td style={{ textAlign: 'center', width: '150px' }}>
                    <EditButton editUrl={`/admin/inquery_details?id=${row.id}`}/>
              ...
              <tr>
                <td colSpan={6}>Inquery List is Empty</td>
              </tr>
            )}
          </tbody>
        </table>       
